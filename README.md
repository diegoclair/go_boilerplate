...
<p align="center">
    <img src="https://skillicons.dev/icons?i=githubactions,postgres,redis,go,docker,prometheus,grafana,jaeger" >
</p>
...
*   **`/infra`**: Provides concrete implementations for interfaces defined in `internal/domain` and `internal/application`. This includes database repositories (PostgreSQL), caching (Redis), logging, etc. It depends on interfaces defined in `/internal` and external libraries/drivers.
...
*   **`.docker`**: Holds Docker volumes data for services like PostgreSQL, Redis and etc.
...
### Configuration
The application loads configuration from `config.toml` by default. You can override settings using environment variables (prefixed with `APP_`). For local development, the `docker-compose.yml` file sets up necessary services (like PostgreSQL, Redis) with default configurations. If you need to customize database credentials or other settings outside of Docker, you can modify `config.toml` or set environment variables.

### ▶️ Launching the Application
To start the application and its dependencies (PostgreSQL, Redis, etc.) using Docker Compose, run the Make command:
```bash
make start
```
...
*   **Real Dependencies with Testcontainers:** For tests involving databases (PostgreSQL) or caches (Redis), we use [Testcontainers](https://testcontainers.com/). This library spins up real dependencies in Docker containers specifically for the test run, providing a high-fidelity testing environment without mocking the database itself.
